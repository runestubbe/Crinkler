...
void ImportHandler::AddImportHunks4K(Part& part, Hunk*& hashHunk, Hunk* header, Hunk* headerRefHunk, const string& rangeDll, bool verbose, bool& enableRangeImport, int& dllSkip) {
	...
	vector<string> dlls;
	for (Hunk* importHunk : importHunks) {
		if (dlls.empty() || dlls.back().compare(importHunk->GetImportDll()) != 0) {
			dlls.emplace_back(importHunk->GetImportDll());
		}
	}
	assert(dlls[0] == "kernel32");

	// Compute DLL name skip distance as longest length of all DLLs
	// except first (kernel32) and last (not skipped)
	dllSkip = 0;
	for (size_t i = 1; i < dlls.size() - 1; i++) {
		dllSkip = max(dllSkip, (int)dlls[i].size() + 1);
	}

	// Build DLLNames hunk
	string dllNames;
	for (size_t i = 1; i < dlls.size(); i++) {
		while (dllNames.size() % dllSkip) {
	...
	// Add DLL names hunk
	Hunk* dllNamesHunk = new Hunk("DLLNamesHunk", dllNames.c_str(), HUNK_IS_WRITEABLE, 0, (int)dllNames.size(), (int)dllNames.size());
	...
	part.RemoveMatchingHunks([](Hunk* hunk) {
		if (hunk->GetFlags() & HUNK_IS_IMPORT) {
			delete hunk;
			return true;
		}
...
void ImportHandler::AddImportHunks1K(Part& part, bool verbose, int& hash_bits, int& max_dll_name_length) {
	if (verbose)
	{
		printf("\n-Imports----------------------------------\n");
	}
	...
	// Fill list for import hunks
	part.ForEachHunk([&found_kernel32, &dll_set, &importHunks](Hunk* hunk) {
		if(hunk->GetFlags() & HUNK_IS_IMPORT)
		{
			hunk = ForwardImport(hunk);
