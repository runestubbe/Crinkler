...
static void PrintRow(back_insert_iterator<vector<char>> out, Hunk& hunk, const int *sizefill, int index, int n, int hex_columns, int hex_width, bool ascii, bool spacing) {
	for(int x = 0; x < hex_columns; x++) {
		if (spacing && x > 0 && (x & 3) == 0) {
			// Spacing between blocks of 4
			format_to(out, "<td byte-spacing>&nbsp;</td>");
		}
		if (spacing && x == 16) {
...
static void HtmlReportRecursive(CompressionReportRecord* csr, back_insert_iterator<vector<char>> out, Hunk& hunk, Hunk& untransformedHunk, const int* sizefill, bool iscode, map<int, Symbol*>& relocs, map<int, Symbol*>& symbols, map<string, int>& opcodeCounters,
	...
	if(csr->type & RECORD_ROOT) {
		...
		format_to(out, "{}", htmlHeader1);
		format_to(out, "<p><b>Report for file {} generated by {} on {}</b></p>\n", exefilename, CRINKLER_WITH_VERSION, asctime(timeinfo));

		// Print option string
		format_to(out, "<p><b>Options:");
		crinkler->PrintOptions(out);
		format_to(out, "</b></p>");

		// Print file size
		format_to(out, "<p><b>Output file size: {}</b></p>", filesize);

		format_to(out, "<p><table><tr><td rowspan='2'><b>Bits per byte:</b></td><td>&nbsp;&nbsp;</td>");
		int ncols = sizeof(sizecols) / sizeof(SizeColor);
		for (int i = 0; i < ncols; i++) {
			format_to(out, "<td class='sc{}' style='background-color:currentColor' colspan='2' title='", i);
			if (i == 0) {
				format_to(out, "Less than {} bits per byte", sizecols[i].text);
			} else if (i == ncols - 1) {
				format_to(out, "More than {} bits per byte", sizecols[i - 1].text);
			} else {
				format_to(out, "Between {} and {} bits per byte", sizecols[i - 1].text, sizecols[i].text);
			}
			format_to(out, "'>&nbsp;&nbsp;&nbsp;&nbsp;</td>");
		}
		format_to(out, "</tr>\n<tr><td colspan='2' align='center'></td>");
		for (int i = 0; i < ncols - 1; i++) {
			format_to(out, "<td colspan='2' align='center'>{}</td>", sizecols[i].text);
		}
		format_to(out, "<td></td></tr></table></p>\n");

		format_to(out, "<style type='text/css'>\n");
		for (int i = 0; i < ncols; i++) {
			format_to(out, ".sc{} {{ color: #{:06X}; }}\n"
				"body.light .sc{} {{ color: #{:06X}; }}\n",
				i, sizecols[i].darkColor, i, sizecols[i].color);
		}
		format_to(out, "</style>\n");

		format_to(out, "{}", htmlHeader2);
	} else {
		...
		format_to(out,"<th nowrap class='c1'>{0}{1:08X}&nbsp;</th>"
					"<th nowrap class='c2'>{2}</th>", divstr.empty() ? "&nbsp;" : "-", CRINKLER_CODEBASE + csr->pos, label);

		if(csr->size > 0) {
			format_to(out, "<th nowrap class='c3'>{}</th>", csr->size);
			if(csr->compressedPos >= 0) {	// Initialized data
				format_to(out,"<th nowrap class='c4'>{:.2f}</th>"
				...
				format_to(out,"<th nowrap class='c4'>&nbsp;</th>"
							"<th nowrap class='c5'>&nbsp;</th>");
			}
		} else {	// Duplicate label
			format_to(out,"<th nowrap class='c3'>&nbsp;</th>"
		...
		if(!divstr.empty())
			format_to(out,"<tr><td nowrap colspan='5'><div id='{}'><table class='grptable'>", divstr);
		else
			format_to(out,"<tr><td nowrap colspan='5'><table class='grptable'>");
			...
			if(csr->compressedPos >= 0 && size > 0) {
				if (iscode) {
					...
					for(int i = 0; i < (int)numinsts; i++) {
						...
						format_to(out, "<tr name='byte' byte='{};{}'>", offset, insts[i].size);
						// Address
						format_to(out, "<td nowrap class='address'>&nbsp;{:08X}&nbsp;</td>", (unsigned int)insts[i].offset);
						
						// Hex dump
						format_to(out, "<td nowrap colspan=4 class='hexdump'><table class='data'><tr>");
					...
					for(int rowLength : rowLengths) {
						// Print address
						format_to(out, "<tr><td nowrap class='address'>&nbsp;{:08X}&nbsp;</td>", CRINKLER_CODEBASE + idx);
	...
	if(csr->type & RECORD_ROOT) {
		format_to(out, "{}", htmlFooter);
	} else {
		format_to(out,"</table>{}</td></tr>", divstr.empty() ? "" : "</div>");
	}
}
...
void HtmlReport(CompressionReportRecord* csr, const char* filename, Hunk& hunk, Hunk& untransformedHunk, const int* sizefill,
	...
	vector<char> out;
	HtmlReportRecursive(csr, back_inserter(out), hunk, untransformedHunk, sizefill, false, relocs, symbols, opcodeCounters,
		exefilename, filesize, crinkler);
